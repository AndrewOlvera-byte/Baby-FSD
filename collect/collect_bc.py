...
def main():
    ...
    try:
        ...
        for ep in range(episodes):
            ...
            while frame_id < max_frames and ep_frames < episode_max_frames:
                world.tick()
                # Refresh agent internal state before checking done(), matching the previously working loop
                try:
                    agent.update_information()
                except Exception:
                    pass
                # If destination reached, stop and end episode before taking another control step
                if agent.done():
                    try:
                ...
                # Buffers for future labels
                speed_mps, yaw_rate = get_ego_state(ego)
                ego_tf_now = ego.get_transform()
                futures.add(sim_time, ego_tf_now, speed_mps)
                ...
                # Ego kinematics and controls (extended)
                ...
                # Enqueue current frame for later labeling once future horizon available
                pending.append({
                    "frame_id": frame_id,
                    "sim_time": sim_time,
                    "ego_tf": ego_tf,
                ...
                # Process matured frames (sufficient future available)
    ...
    try:
        bev_paths = [os.path.join(run_dir, p) for p in os.listdir(run_dir) if p.startswith('bev_frames-') and p.endswith('.parquet')]
        bev_paths.sort()
        if bev_paths:
            import io
